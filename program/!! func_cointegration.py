...
----------------------------------------------------------------------------------------------------
#second dump -20240229

# Make sure to have the constants.py file in the same directory with RSQUARED_THRESH defined among other constants.
import pandas as pd
import numpy as np
import statsmodels.api as sm
from statsmodels.tsa.stattools import coint
from constants import MAX_HALF_LIFE, WINDOW, RSQUARED_THRESH
...
def calculate_cointegration(series_1, series_2):
    ...
    t_check = coint_t < critical_value
    coint_flag = 1 if p_value < 0.05 and t_check else 0
    return coint_flag, hedge_ratio, half_life, r_squared
...
def store_cointegration_results(df_market_prices):
    ...
    for index, base_market in enumerate(markets[:-1]):
        ...
        for quote_market in markets[index + 1:]:
            ...
            if len(series_1) < WINDOW or len(series_2) < WINDOW:
                continue

            coint_flag, hedge_ratio, half_life, r_squared = calculate_cointegration(series_1, series_2)
            if coint_flag == 1 and half_life <= MAX_HALF_LIFE and half_life > 0 and r_squared >= RSQUARED_THRESH:
                criteria_met_pairs.append({
                    "base_market": base_market,
                    "quote_market": quote_market,
                    "hedge_ratio": hedge_ratio,
